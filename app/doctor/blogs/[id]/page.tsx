  ...
  return (
    <div className="p-6 max-w-4xl mx-auto flex flex-col gap-5">
      <div className="flex items-center justify-between">
        ...
        <Skel className="w-32 h-10 rounded-xl" />
      </div>
      ...
      <Skel className="w-full h-64 rounded-2xl" />
    </div>
  );
}
...
export default function DoctorBlogDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/doctor/blogs/${id}`).then(r => r.json()),
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/doctor/blogs/${id}`, {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    await fetch(`/api/doctor/blogs/${id}`, { method: 'DELETE' });
    ...
    <div className="p-6 max-w-4xl mx-auto flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 min-w-0">
          <button onClick={() => router.push('/doctor/blogs')} className="p-2 rounded-xl hover:bg-gray-100 text-gray-500 shrink-0 transition-colors">
            <ArrowLeft size={18} />
          </button>
      ...
      <div className="flex gap-3 pb-2">
        <button onClick={() => router.push('/doctor/blogs')}
