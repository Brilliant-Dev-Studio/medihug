...
export default function NewDoctorBlogPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/doctor/blogs', {
    ...
    <div className="p-6 max-w-4xl mx-auto flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-800">Write Blog</h1>
            <p className="text-xs text-gray-400">{step === 1 ? 'Fill in the article details' : 'Write your story in Markdown'}</p>
          </div>
        </div>
        <div className="flex gap-3">
          <button onClick={() => router.back()} className="px-5 py-2.5 rounded-xl border border-gray-200 text-sm font-bold text-gray-500 hover:bg-gray-50 hover:border-gray-300 transition-colors">
            Cancel
          </button>
          {step === 1 ? (
            <button onClick={goToStep2}
              className="px-5 py-2.5 rounded-xl text-white text-sm font-bold flex items-center justify-center gap-2 shadow-[0_4px_14px_-4px_rgba(42,181,173,0.5)] hover:shadow-[0_6px_18px_-4px_rgba(42,181,173,0.6)] hover:-translate-y-px active:translate-y-0 transition-all"
              style={{ background: `linear-gradient(135deg, ${PRIMARY} 0%, ${DARK} 100%)` }}>
              Next: Write Content <ArrowRight size={16} />
            </button>
          ) : (
            <button onClick={handleSubmit} disabled={loading}
              className="px-5 py-2.5 rounded-xl text-white text-sm font-bold flex items-center justify-center gap-2 disabled:opacity-60 shadow-[0_4px_14px_-4px_rgba(42,181,173,0.5)] hover:shadow-[0_6px_18px_-4px_rgba(42,181,173,0.6)] hover:-translate-y-px active:translate-y-0 transition-all"
              style={{ background: `linear-gradient(135deg, ${PRIMARY} 0%, ${DARK} 100%)` }}>
          ...
          >
            ...
            <div className="flex gap-3 pb-2">
              <button onClick={() => router.back()}
                className="flex-none px-6 py-3 rounded-xl border border-gray-200 text-sm font-bold text-gray-500 hover:bg-gray-50 hover:border-gray-300 transition-colors">
                Cancel
              </button>
              <button onClick={goToStep2}
                className="flex-1 py-3 rounded-xl text-white text-sm font-bold flex items-center justify-center gap-2 shadow-[0_4px_14px_-4px_rgba(42,181,173,0.5)] hover:shadow-[0_6px_18px_-4px_rgba(42,181,173,0.6)] hover:-translate-y-px active:translate-y-0 transition-all"
                style={{ background: `linear-gradient(135deg, ${PRIMARY} 0%, ${DARK} 100%)` }}>
                Next: Write Content <ArrowRight size={16} />
              </button>
            </div>
          ...
          >
            ...
            <div className="flex gap-3 pb-2">
              <button onClick={() => setStep(1)}
                className="flex-none px-6 py-3 rounded-xl border border-gray-200 text-sm font-bold text-gray-500 hover:bg-gray-50 hover:border-gray-300 transition-colors flex items-center gap-2">
                <ArrowLeft size={16} /> Back
              </button>
              <button onClick={handleSubmit} disabled={loading}
                className="flex-1 py-3 rounded-xl text-white text-sm font-bold flex items-center justify-center gap-2 disabled:opacity-60 shadow-[0_4px_14px_-4px_rgba(42,181,173,0.5)] hover:shadow-[0_6px_18px_-4px_rgba(42,181,173,0.6)] hover:-translate-y-px active:translate-y-0 transition-all"
                style={{ background: `linear-gradient(135deg, ${PRIMARY} 0%, ${DARK} 100%)` }}>
                {loading ? <Loader2 size={16} className="animate-spin" /> : <Rocket size={15} />}
                {loading ? 'Saving...' : (form.isPublished ? 'Publish Blog' : 'Save Draft')}
              </button>
