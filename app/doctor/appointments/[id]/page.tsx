...
import {
  ...
  NotebookPen, Share2, Search, X, Building2, Save,
} from 'lucide-react';
...
function ReferralPicker<T extends { id: string; name: string; nameEn: string | null; imageUrl: string | null }>({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      search(query).then(r => { setResults(r); setSearching(false); }).catch(() => setSearching(false));
    }, 300);
...
/** Doctor-written clinical note (Markdown) + referral-to-doctor / referral-to-partner-clinic for this appointment. */
function NoteReferralCard({ appt, mm, onSaved }: {
  ...
  const searchDoctors = async (q: string): Promise<DoctorHit[]> => {
  ...
  const searchClinics = async (q: string): Promise<ClinicHit[]> => {
    const res = await fetch(`/api/clinics?search=${encodeURIComponent(q)}&limit=8`);
    const data = await res.json();
    return data.clinics ?? [];
  };
