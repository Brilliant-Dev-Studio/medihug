        ...
        <div className="flex flex-wrap gap-1.5">
          {tags.map(t => (
            <span key={t} className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-medium">
              {t}
              <button type="button" onClick={() => onChange(tags.filter(x => x !== t))} className="text-blue-400 hover:text-blue-700"><X size={11} /></button>
            </span>
          ))}
        </div>
      )}
...
export default function PartnerEditProductPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const [form, setForm]             = useState({
    ...
    price: 0, priceThb: '' as number | string, priceUsd: '' as number | string, stock: 0,
    images: [] as string[], category: '', isActive: true,
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/partner/products/${id}`).then(r => r.ok ? r.json() : Promise.reject(r.status)),
    ...
    <div className="p-4 lg:p-6 max-w-5xl mx-auto flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3 min-w-0">
          <button onClick={() => router.push('/partner/products')} className="p-2 rounded-xl hover:bg-gray-100 text-gray-500 shrink-0"><ArrowLeft size={18} /></button>
          <div className="flex items-center gap-3 min-w-0">
            {product.imageUrl
      ...
      <div className="flex justify-end gap-3 pb-2">
        <button onClick={() => router.push('/partner/products')} className="px-6 py-3 rounded-xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50">Cancel</button>
        <button onClick={handleSave} disabled={saving}
