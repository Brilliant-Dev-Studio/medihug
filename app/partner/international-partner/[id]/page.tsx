...
export default function EditSubClinicPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/partner/sub-clinics/${id}`).then(r => r.json()),
      fetch('/api/partner-types').then(r => r.json()),
    ]).then(([sd, ptd]) => {
      setPartnerTypes(ptd.partnerTypes ?? []);
      const c: SubClinic | undefined = sd.subClinic;
      if (!c) { setError('Not found'); setLoading(false); return; }
      setForm({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`/api/partner/sub-clinics/${id}`, {
    ...
    <div className="p-4 lg:p-6 max-w-3xl mx-auto flex flex-col gap-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-800">Edit International Partner</h1>
            <p className="text-xs text-gray-400">{form.nameEn || form.name}</p>
          </div>
        </div>
        <div className="flex gap-3">
          <button onClick={() => router.push('/partner/international-partner')}
      ...
      <Section title="Location">
        ...
        <div>
          ...
          <input className={inp} value={form.addressEn} onChange={e => set('addressEn', e.target.value)} />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
      ...
      <Section title="About">
        <div>
          ...
          <textarea className={inp + ' resize-none'} rows={5} value={form.aboutMm} onChange={e => set('aboutMm', e.target.value)} />
        </div>
        <div>
          <label className={lbl}>About (English)</label>
          <textarea className={inp + ' resize-none'} rows={5} value={form.aboutEn} onChange={e => set('aboutEn', e.target.value)} />
        </div>
      </Section>
