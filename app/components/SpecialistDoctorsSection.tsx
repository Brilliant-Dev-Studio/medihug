...
function SkeletonTile({ mobile = false }: { mobile?: boolean }) {
  return (
    <div className={`flex items-center justify-between gap-2 bg-white rounded-xl border border-gray-100 ${mobile ? 'px-2.5 py-2' : 'px-3 py-2.5'}`}>
      <div className="h-3 bg-gray-200 rounded animate-pulse flex-1" />
      <div className={`${mobile ? 'w-8 h-8' : 'w-9 h-9'} rounded-lg bg-gray-100 animate-pulse shrink-0`} />
    </div>
  );
}
            ...
            : shown.map((s, i) => {
                ...
                return (
                  <Link key={s.id}
                    ...
                    className="flex items-center justify-between gap-2 bg-white rounded-xl border border-gray-100 px-3 py-2.5 active:scale-[0.98] transition-all hover:border-gray-200 hover:shadow-sm">
          ...
          {loading
            ...
            : shown.map((s, i) => {
                ...
                return (
                  <Link key={s.id}
                    href={`/patient/doctors?specialty=${encodeURIComponent(s.name)}`}
                    className="flex items-center justify-between gap-1.5 bg-white rounded-xl border border-gray-100 px-2.5 py-2 active:scale-[0.98] transition-all">
                    <span className="text-[11px] font-semibold text-gray-700 leading-snug flex-1">{label}</span>
