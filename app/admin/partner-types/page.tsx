...
export default function AdminPartnerTypesPage() {
  ...
  const load = useCallback(async (p = page) => {
    ...
    const res  = await fetch(`/api/admin/partner-types?${q}`);
  ...
  const handleDelete = async (id: string) => {
    setDeletingId(id);
    await fetch(`/api/admin/partner-types/${id}`, { method: 'DELETE' });
    setDeletingId(null);
    load(page);
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-800">Partner Types</h1>
          <p className="text-sm text-gray-400 mt-0.5">{total} partner types</p>
        </div>
        {!creating && (
          <button
      ...
      {creating && (
        <div className="bg-white rounded-2xl border-2 p-4 flex flex-col gap-3" style={{ borderColor: PRIMARY }}>
          <p className="text-xs font-bold uppercase tracking-widest" style={{ color: PRIMARY }}>New Partner Type</p>
          <div className="flex gap-2">
            <input
              autoFocus
              value={newName}
