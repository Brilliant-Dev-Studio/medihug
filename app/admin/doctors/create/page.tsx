...
import { ArrowLeft, ChevronDown, X, Clock, Loader2, Check, User, CalendarClock, Copy, Minus, Plus, Images } from 'lucide-react';
...
export default function CreateDoctorPage() {
  ...
  const applyToAll = (source: Slot) =>
    setSlots(s => s.map(sl => ({ ...sl, startTime: source.startTime, endTime: source.endTime, maxPerSlot: source.maxPerSlot })));
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res  = await fetch('/api/admin/doctors', {
        ...
        body: JSON.stringify({ ...form, slots, gallery }),
      });
                ...
                <div key={slot.dayOfWeek} className="rounded-2xl border border-gray-100 overflow-hidden">
                  ...
                  <div className="p-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <div><label className={lbl}>Duration</label>
                      <div className={inp + ' flex items-center gap-1.5 text-gray-500'}>
                    ...
                    <div><label className={lbl}>Max per Slot</label>
                      <div className="flex items-center bg-gray-50 border border-gray-200 rounded-xl overflow-hidden">
                        ...
                        <input type="number" min={1} max={10} value={slot.maxPerSlot} readOnly
                          className="flex-1 min-w-0 bg-transparent text-center text-sm font-semibold text-gray-700 outline-none" />
                        <button type="button" onClick={() => updateSlot(slot.dayOfWeek,'maxPerSlot', Math.min(10, slot.maxPerSlot + 1))}
                          className="w-8 h-full py-2.5 flex items-center justify-center text-gray-400 hover:text-gray-600 transition-colors shrink-0">
